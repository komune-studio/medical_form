...
const Login = () => {
    ...
    const onSubmit = async (values) => {
        try {
            ...
            localStorage.setItem('super_token', result.token);
            localStorage.setItem('admin_name', result.username);
            localStorage.setItem('role', result.role);           // 'ADMIN' atau 'DOCTOR'
    ...
    return (
        <>
            <style>{`
                ...
                .login-input .ant-input:hover { border-color: #666 !important; }
                .login-input .ant-input:focus {
                    border-color: #666 !important;
                    box-shadow: 0 0 0 2px rgba(102,102,102,0.1) !important;
                }
                .anticon-user, .anticon-lock { color: #333 !important; }
                ...
                <Card style={{
                    ...
                    <CardBody className="px-5 py-5">
                        <div className="text-center mb-4">
                            <Typography.Title
                                level={2}
                                style={{ color: '#333', fontWeight: 600, marginBottom: '10px' }}
                            >
                                Rangka
                            </Typography.Title>
